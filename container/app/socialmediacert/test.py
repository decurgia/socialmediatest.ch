...
def index():
    db = get_db()
    tests = db.execute(
        "SELECT test_id, name, description, number_of_questions, pass_quota FROM test"
    ).fetchall()
...
def question():
    ...
    if request.method == "POST":
        ...
        db = get_db()
        question = db.execute(
            f"SELECT question_id FROM question WHERE question_id = {questions[0]} AND answer = {answer}"
        ).fetchone()
        # evaluate answer
        if question:
...
def result():
    ...
    db = get_db()
    test = db.execute(
        f"SELECT test_id, name, number_of_questions, pass_quota FROM test WHERE test_id = {test_id}"
    ).fetchone()
    ...
    return render_template("test/result.html", test_quota=test_quota)
